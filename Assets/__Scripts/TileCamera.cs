...
public class TileCamera : MonoBehaviour
{
    ...
    public void LoadMap()
    {
        ...
        //Сохранить информацию для карты в двухмерный массив для ускорения доступа
        MAP = new int[W, H];
        for(int j=0;j<H; j++)
        {
    ...
    static public int GET_MAP (int x, int y)
    {
        if(x<0 || x>=W || y<0 || y >= H)
        {
            return -1; //предотвратить исключение IndexOutRangeException
        }
        return MAP[x, y];
    }
    ...
    static public void SET_MAP (int x, int y, int tNum)       //f
    {
        ...
        if( x<0 || x>=W || y<0 || y>= H)
        {
            return;  //предотвратить исключение IndexOutRangeException
        }
        MAP[x, y] = tNum;
    }
}
